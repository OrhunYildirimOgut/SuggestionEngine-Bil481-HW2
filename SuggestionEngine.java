...
public class SuggestionEngine extends Java8BaseListener {
	...
	public void enterMethodDeclaration(Java8Parser.MethodDeclarationContext ctx){
		for(Java8Parser.MethodModifierContext currentMethod : ctx.methodModifier()){
			if(currentMethod.getText().toString().equals("public")){
				String nameOfTheMethod = ctx.methodHeader().methodDeclarator().Identifier().getText();
            	mMethods.add(nameOfTheMethod);
			}
		}
	}
	...
	private TreeSet<Candidate> getTopKNeighbor(String word,int K) {
		...
		for(int j=0;j < mMethods.size(); j= j +1){
			String nameOfTheMethod= mMethods.get(j);
			double distance = Levenshtein.distance(word, nameOfTheMethod);
			Candidate candidateInstance = new Candidate(nameOfTheMethod, distance);
			minHeap.add(candidateInstance);
			if(minHeap.size() > K){
